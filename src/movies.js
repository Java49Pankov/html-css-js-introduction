...
const sectionPopularity = document.querySelector(".details-popularity");
const sectionLeastPopularity = document.querySelector(".details-least");

ulElement.innerHTML = getPosterImages();
sectionPopularity.innerHTML = getMostPopularity();
sectionLeastPopularity.innerHTML = getLeastPopularity();


function createElement(movie) {
    return `<li class="movie-item">
    <img class="movie-image" src="${httpPrefix}${movie.poster_path}">
    <div class="details-title">${movie.original_title}</div>
...
function sortMovies() {
    const arPop = moviesData.results.sort((a, b) => {
...
function getMostPopularity() {
    const arPopul = sortMovies();
    return createElement(arPopul.shift());
}

function getLeastPopularity() {
    const arPopul = sortMovies();
    return createElement(arPopul.pop());
}
